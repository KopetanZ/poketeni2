// 部員配属システム
// 作成日: 2025-01-13
// 目的: PokeAPIデータに基づく部員配属と個体値システム

import { convertStatsToTennisSkills, applyIndividualValues, TennisSkills, PokemonBaseStats } from './stats-conversion';
...
export interface PokemonMember {
  ...
  growth_efficiency: Record<string, number>;
...
// 個体値生成システム
export function generateIndividualValues(): Record<string, number> {
  return {
...
// 成長効率の個体差生成
export function generateGrowthEfficiencyVariance(): Record<string, number> {
  return {
...
// レアリティ判定
export function determineRarity(baseStats: PokemonBaseStats, types: string[]): 'common' | 'uncommon' | 'rare' | 'epic' | 'legendary' {
...
// 進化データ取得
export function getEvolutionData(pokemonId: number): any {
  // 進化チェーンデータベースから取得
  // 実際の実装ではデータベースクエリが必要
  return null; // 仮実装
}
...
export function assignRandomAbilities(
  ...
  if (rarity === 'legendary' && Math.random() < 0.3) {
    // 2つ目の能力付与処理
    const secondAbility = TENNIS_SPECIAL_ABILITIES.find(ability => 
      !assignedAbilities.includes(ability.id) && ability.color === 'gold'
    );
    if (secondAbility) {
      assignedAbilities.push(secondAbility.id);
    }
  }
...
// 能力タイプ決定
function determineAbilityType(rates: any): string {
  const rand = Math.random() * 100;
  if (rand < rates.goldAbility) return 'gold';
  if (rand < rates.goldAbility + rates.blueAbility) return 'blue';
  if (rand < rates.goldAbility + rates.blueAbility + rates.redAbility) return 'red';
  return 'blue'; // デフォルト
}
...
// テニス特殊能力データ（仮実装）
const TENNIS_SPECIAL_ABILITIES = [
  { id: 'power_serve', name: 'パワーサーブ', color: 'gold', description: 'サーブの威力が大幅に向上' },
  { id: 'quick_return', name: 'クイックリターン', color: 'blue', description: 'リターンの反応速度が向上' },
  { id: 'net_master', name: 'ネットマスター', color: 'blue', description: 'ボレー技術が向上' },
  { id: 'steady_stroke', name: 'ステディストローク', color: 'blue', description: 'ストロークの安定性が向上' },
  { id: 'mental_fortress', name: 'メンタルフォートレス', color: 'blue', description: 'メンタルが向上' },
  { id: 'endurance', name: 'エンデュランス', color: 'blue', description: 'スタミナが向上' },
  { id: 'serve_anxiety', name: 'サーブ不安', color: 'red', description: 'サーブの成功率が低下' },
  { id: 'return_weakness', name: 'リターン弱点', color: 'red', description: 'リターンの成功率が低下' },
];

// 部員配属メイン処理
export async function recruitNewMember(
  pokemonMasterData: any,
  supabase: any
): Promise<RecruitmentResult> {
  try {
    // 1. 基本ステータス計算
    const baseStats: PokemonBaseStats = {
      id: pokemonMasterData.pokemon_id,
      hp: pokemonMasterData.base_stats.hp,
      attack: pokemonMasterData.base_stats.attack,
      defense: pokemonMasterData.base_stats.defense,
      sp_attack: pokemonMasterData.base_stats.sp_attack,
      sp_defense: pokemonMasterData.base_stats.sp_defense,
      speed: pokemonMasterData.base_stats.speed
    };
    
    const baseSkills = convertStatsToTennisSkills(baseStats, pokemonMasterData.types);
    ...
    // 4. 特殊能力付与
    const rarity = determineRarity(baseStats, pokemonMasterData.types);
    const specialAbilities = assignRandomAbilities({ skills: finalSkills }, rarity);
    
    // 5. メンバーオブジェクト作成
    const newMember: PokemonMember = {
      pokemon_name: pokemonMasterData.japanese_name,
      pokemon_id: pokemonMasterData.pokemon_id,
      english_name: pokemonMasterData.english_name,
      types: pokemonMasterData.types,
      ...finalSkills,
      individual_values: individualValues,
      growth_efficiency: growthEfficiency,
      special_abilities: specialAbilities,
      rarity_level: rarity,
      can_evolve: canEvolve(pokemonMasterData.pokemon_id),
      evolution_data: getEvolutionData(pokemonMasterData.pokemon_id),
    };
    ...
    return {
      success: true,
      member: data,
      message: `${pokemonMasterData.japanese_name}の配属が完了しました！`
    };
...
// 配属可能ポケモン一覧取得
export async function getRecruitablePokemon(supabase: any) {
  try {
    const { data, error } = await supabase
...
// ポケモン詳細情報取得
export async function getPokemonDetails(pokemonId: number, supabase: any) {
  try {
    const { data, error } = await supabase
      .from('pokemon_master_data')
      .select(`
